...
export function nearestDate(dates: string[], today = new Date().toISOString().slice(0,10)) { if(!dates.length) return today; return [...dates].sort((a,b)=>Math.abs(new Date(a).getTime()-new Date(today).getTime())-Math.abs(new Date(b).getTime()-new Date(today).getTime()))[0]; }
